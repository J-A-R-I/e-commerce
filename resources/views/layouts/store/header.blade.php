...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <x-app-logo href="{{ route('backend') }}" wire:navigate />

        <flux:navbar class="-mb-px max-lg:hidden">
            <flux:navbar.item icon="layout-grid" :href="route('backend')" :current="request()->routeIs('backend')"
                wire:navigate>
                {{ __('backend') }}
            </flux:navbar.item>
        </flux:navbar>
    ...
    <flux:sidebar collapsible="mobile" sticky
        ...
        <flux:sidebar.nav>
            <flux:sidebar.group :heading="__('Platform')">
                <flux:sidebar.item icon="layout-grid" :href="route('backend')" :current="request()->routeIs('backend')"
                    wire:navigate>
                    {{ __('backend') }}
                </flux:sidebar.item>
            </flux:sidebar.group>
